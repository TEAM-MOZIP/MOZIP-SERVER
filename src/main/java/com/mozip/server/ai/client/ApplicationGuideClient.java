...
public class ApplicationGuideClient {

    private static final String GENERATE_URI = "/api/v1/guides/generate";

    private final RestClient restClient;

    @Autowired
    public ApplicationGuideClient(RestClient.Builder restClientBuilder, MozipAiProperties properties) {
     ...
     * 덮어쓰지 않는다.
     */
    ApplicationGuideClient(RestClient restClient) {
    ...
    public ApplicationGuideResponse generate(ApplicationGuideRequest request) {
        return restClient.post()
                .uri(GENERATE_URI)
                .body(request)
                .retrieve()
                .body(ApplicationGuideResponse.class);
    }
}
